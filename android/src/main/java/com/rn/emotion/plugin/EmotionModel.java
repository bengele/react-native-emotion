package com.rn.emotion.plugin;


import com.rn.emotion.R;
...
public class EmotionModel {
    ...
    public static List<EmotionNote> getEmojis() {
        List<EmotionNote> list = new ArrayList<>();

        Iterator<Map.Entry<String, Integer>> entries = EMOTION_CLASSIC_MAP.entrySet().iterator();

        while (entries.hasNext()) {
            Map.Entry<String, Integer> entry = entries.next();
            list.add(new EmotionNote(entry.getKey(),entry.getValue()));
        }

        return list;
    }

   public static class EmotionNote {

        private String mText;
        @DrawableRes
        private int mIconRes;
        EmotionNote(String text, @DrawableRes int res) {
            mText = text;
            mIconRes = res;
        }
